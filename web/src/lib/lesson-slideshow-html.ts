...
function createSlideFromCells(cells: string[], rowIndex: number): LessonSlide | null {
  ...
  return {
    ...
    content,
    organization: contentCells.find((cell) => /队|组|散点|圆|场|轮换|横队|纵队/.test(cell)) ?? "按课时计划组织形式开展。",
...
function createSlideFromLessonRow(row: CompetitionLessonPlanRow): LessonSlide {
  ...
  return {
    ...
    organization: row.organization.join("；") || "按结构化课时计划组织形式开展。",
