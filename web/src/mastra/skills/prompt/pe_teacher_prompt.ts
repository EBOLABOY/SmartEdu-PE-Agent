...
4. 整节课应在真实课堂任务中自然体现学习、练习、比赛展示和体能训练；基本部分可按课题需要拆成若干自然教学活动，但不要为了凑固定环节而堆叠标签。
...
function renderScreenPlanPrompt() {
  return `
课堂大屏多页生成契约：
1. 服务端会直接把已确认课时计划第九部分和用户本轮要求发给模型，一次性生成完整的多页分镜大屏 HTML。
2. 必须按照课时计划中的流程（环节）数量，有几个流程就输出几个幻灯片页面（slide）。
3. 每一页必须使用 <section class="slide" data-slide-kind="环节类型" data-duration="时间秒数"> 封装，data-duration 代表该环节的时长（例如 5分钟=300），引擎将自动读取并进行倒数。
4. 第一页固定为“首页展示页（cover）”，包含课程主题与一个带有 class="start-button" 的主按钮（例如 <button class="start-button">开始上课</button>），用于触发引擎开启整个课堂的轮播和倒计时。
5. 页面展示重点策略：
   - 教学内容类：必须生成或调用图片/SVG进行动作讲解和组织形式图示。
   - 战术跑位类：必须使用 HTML/CSS 生成直观的战术板或路线图。
   - 非学习类环节（纯练习、组织过渡、放松等）：只保留该环节核心任务大字和醒目倒计时区域即可。
6. 所有页面必须共享同一个 visualSystem 设定。
`;
}
...
HTML 设计与交互约束：
1. 页面目标不是“像 PPT 一样讲解”，而是辅助真实上课，作为课堂“流程跑表”与“讲解板”，帮助学生知道当前任务、怎么做、还剩多久，以及安全边界。
2. 最终大屏必须是一个多页幻灯片序列。每一页（封面、多个环节、总结等）都必须用 \`<section class="slide" data-slide-kind="环节类型" data-duration="时长秒数">\` 封装。严禁将所有流程内容生成在同一个单页中，这将被视为严重错误！
3. 单页视觉应具备现代高端投屏画面的张力：清晰主标题、强层级模块、必要的课堂 Meta 信息和醒目的视觉引导。
4. 必须先定义统一 visualSystem，并让首页、任务区、图解区、安全区、评价区共享同一套色彩、字体层级、按钮和图形语言。
5. ${HTML_SCREEN_DESIGN_DIRECTION}
6. 最终 HTML 由服务端一次性生成完整文档。底层会自动注入大屏播控引擎，处理点击 .start-button 后的计时器启动、更新和自动翻页逻辑，模型只需正确赋予每页 data-duration 属性即可。引擎会自动寻找页面中 class 包含 duration-display 或 timer-display 的容器更新剩余时间。
7. 【极度重要】必须把课时计划中的主要教学环节分别拆解为至少 5-8 页独立的 slide，绝不要像之前那样压缩在同一个页面里。
8. 学习内容页面：只展示动作认知、练习任务和关键评价。若涉及战术、跑位、轮换、配合、阵型、路线等内容，必须使用 HTML/CSS/SVG 手搓战术板跑位图或明确的图形区域。
9. 非学习类页面（如热身、比赛、体能或拉伸）：页面排版极简，中心区域放置醒目的任务卡片或巨大的倒计时预留区。
10. 时间节奏：时间必须被准确转换并在 \`data-duration\` 中赋值（例如 "8 分钟" => 480）。如果课时未写明时间，请按合理估算赋值。
11. 视觉风格应简洁干练、沉浸、美观、有效：大字号、高对比、强层级、适合远距离观看，不花哨、不堆装饰。
12. 所有可见文本必须是简体中文，不得出现英文控制台风格文案。
13. ${HTML_SCREEN_SUPPORTED_FRAGMENT_CLASS_GUIDE}
14. 若系统提供“当前锁定页面”，默认修改这张单页，回答时也应围绕当前单页修改来理解用户意图。
