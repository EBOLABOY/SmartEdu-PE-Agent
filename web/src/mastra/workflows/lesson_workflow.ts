...
const standardsRetrievalOutputSchema = intakeCollectionOutputSchema.extend({
  standardsContext: z.string(),
...
function buildStandardsWorkflowFields(inputData: Pick<z.infer<typeof lessonWorkflowInputSchema>, "market" | "query">) {
  ...
  return {
    ...
    trace: [
      createTraceEntry(
        ...
        "success",
        `目标市场 ${standardsResult.requestedMarket} 已解析为 ${standardsResult.resolvedMarket}，命中 ${standardsResult.references.length} 条课标条目。`,
      ),
...
function buildWorkflowWarnings(inputData: {
  ...
  return [
    ...
    ...(inputData.standards.referenceCount === 0
...
const prepareClarificationResponseStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    const standards = buildStandardsWorkflowFields(inputData);
...
const retrieveStandardsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    return {
      ...inputData,
      ...standards,
      trace: [...inputData.trace, ...standards.trace],
    };
  },
});
    ...
    return {
      ...
      trace: [
        ...
        createTraceEntry(
          ...
          "success",
          `已构造 ${inputData.mode} 阶段系统提示词，并注入信息收集、目标市场与课标上下文。`,
        ),
      ],
    };
  },
