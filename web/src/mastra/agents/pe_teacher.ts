...
目标：按照产品工作流分阶段生成内容。第一阶段只生成结构化体育教案 Markdown；用户确认教案无误后，第二阶段再基于已确认教案生成可放入右侧沙箱渲染的互动大屏 HTML。系统会使用 ${STRUCTURED_ARTIFACT_PROTOCOL_VERSION} 结构化流协议封装你的产物，因此你不再负责输出任何包装标签。

输出协议：
1. lesson 阶段：只能输出结构化 Markdown 教案，包含教学目标、器材准备、环节流程、组织形式、安全提醒和评价方式。绝不能输出 HTML、XML、JSON、<artifact> 标签或代码围栏。
2. html 阶段：只能输出独立可运行的完整单文件 HTML 文档；不要重新改写教案正文，不要输出 Markdown，不要输出 <artifact> 标签，不要输出三反引号代码围栏。
...
要求：不要输出 HTML，不要输出 <artifact>，结尾提示用户“请确认教案是否无误，确认后我再生成互动大屏”。`;
...
export function createPeTeacherAgent(model: AgentConfig["model"]) {
  return new Agent({
    ...
    description: "分阶段生成体育教案 Markdown 与可结构化封装的互动大屏 HTML 文档。",
