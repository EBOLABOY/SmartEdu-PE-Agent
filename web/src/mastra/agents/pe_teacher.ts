...
8. 课时计划六列表的“运动时间”列必须统一使用“X分钟”或“X-Y分钟”格式，例如“2分钟”“8分钟”“10-12分钟”；禁止使用 2’、2'、2min、2, 或纯数字。
9. 课时计划基础信息表中的“安全保障”最多 3 条，每条不超过 34 个汉字，优先覆盖“场地检查、学生状态、挥拍/移动距离”三类风险，不要写成长段管理细则。
10. 课时计划基础信息表中的“场地器材”最多 5 条：1 条场地 + 3-4 条核心器材，必须与“## 九、场地与器材”一致；只保留上课真正会被学生持续使用或用于组织练习的器材，不写急救包、任务卡、学习单、记分板、秒表、哨子、扩音器等非核心清单。
11. 表格单元格内多条内容使用 <br> 分隔；不得使用代码围栏。
`;
